...
typealias CellCornerSetting = (isTopCorner: Bool, isBottomCorner: Bool)

class NewsHeadlinesCell: UICollectionViewCell {
    ...
    var cornerSetting: CellCornerSetting? {
        didSet {
            var corners = CACornerMask()
            if let setting = cornerSetting {
                if setting.isTopCorner {
                    corners.insert(.layerMaxXMinYCorner)
                    corners.insert(.layerMinXMinYCorner)
                }
                if setting.isBottomCorner {
                    corners.insert(.layerMinXMaxYCorner)
                    corners.insert(.layerMaxXMaxYCorner)
                }
            }
            self.layer.cornerRadius = corners.isEmpty ? 0 : 15
    ...
    func updateArticleInfo(activeVC: UIViewController, article: Article, cornerSetting: CellCornerSetting? = nil) {
        self.activeVC = activeVC
        self.delegate = activeVC as? any NewsCellDelegate
        self.article = article
        self.cornerSetting = cornerSetting
    }
    
    func updateImageArticleInfo(activeVC: UIViewController, article: Article, cornerSetting: CellCornerSetting? = nil) {
        self.activeVC = activeVC
        self.delegate = activeVC as? any NewsCellDelegate
        self.article = article
        self.cornerSetting = cornerSetting
...
class NewsHeadlinesImageCell: NewsHeadlinesCell {
    
}

class NewsMarkListCell: UITableViewCell {
